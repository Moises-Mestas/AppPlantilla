...
class IngresoViewModel(app: Application) : AndroidViewModel(app) {
    private val _navigateToCuentasEvent = MutableStateFlow(false)
    val navigateToCuentasEvent: StateFlow<Boolean> = _navigateToCuentasEvent
    ...
    // Al guardar o editar el ingreso, emites el evento de éxito
    fun save(isGasto: Boolean = false) {
        viewModelScope.launch {
            ...
            try {
                ...
                // Emitir evento de éxito (navegar a la pantalla de Cuentas)
                _navigateToCuentasEvent.value = true
    ...
    // Resetear navegación
    fun resetNavigation() {
        _navigateToCuentasEvent.value = false
    }
